...
delta_freq = 00e6; % Offset del LO
theta0 = 0/180*pi; % Fase inicial
LW = 00e3; % Ancho de linea [Hz] -> Ruido de fase
frequency_fluctuations_amp = 0e6;
frequency_fluctuations_freq = 1e6;
phase_tone_amp = 10/180*pi;
phase_tone_freq = 5000e6;
...
for m = 2:Ldata
    ...
    % Estimacion del error de fase instantaneo
    a_hat = my_slicer(derot_x, M);
    phase_error(m) = angle(derot_x .* conj(a_hat));
    
    % Rama proporcional del filtro de lazo
    prop_error = phase_error(m) * Kp;
    
    % Rama integral del filtro de lazo
...
plot(unwrap(angle(phase_tone)),'--'); hold all
grid on
xlabel("Time")
ylabel("Phase [rad]")
...
for m=1:Ldata
    orx_cs_fixed_slicer(m) = my_slicer(orx_cs_fixed(m), M);
end
